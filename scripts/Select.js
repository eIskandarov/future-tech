...
class Select extends BaseComponent {
  ...
  updateUI() {
    ...
    const updateButton = () => {
      ...
      this.buttonElement.setAttribute(this.stateAttributes.ariaSelected, isExpanded);
    ...
    const updateOptions = () => {
      this.optionElements.forEach((optionElement, index) => {
        const isCurernt = currentOptionIndex === index;
        const isSelected = selectedOptionElement === optionElement;

        optionElement.classList.toggle(this.stateClasses.isCurrent, isCurernt);
...
class SelectCollection {
  ...
  init() {
    document.querySelectorAll(rootSelector).forEach(elem => new Select(elem));
  }
}
